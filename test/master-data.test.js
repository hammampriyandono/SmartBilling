...
test('PostgreSQL master: scope owner, audit, version, move atomik, simulation dan histori',{
 ...
 try{
  ...
  let response=await call('/api/owner/rooms','POST',createRoom,createKey);assert.equal(response.status,201);let room=(await response.json()).data;
  ...
  assert.equal((await db.query('SELECT count(*)::int n FROM audit_logs WHERE property_id=$1',[property])).rows[0].n,7);
  ...
  acting={id:other,role:'owner'};assert.deepEqual((await (await call('/api/owner/rooms')).json()).data,[]);assert.equal((await call(`/api/owner/rooms/${room.id}`,'PATCH',{name:'Bocor',row_version:2,reason:'uji'})).status,404);
  acting={id:tenant,role:'tenant'};assert.equal((await call('/api/owner/rooms')).status,403);
